...
class Plateforme
{
      ...
      Cours getCours(std::string nom); //recupere le cours avec son nom
      Utilisateur getUti(std::string login); // recupere l'uti avec son login
      bool existeLogin(std::string login);
      std::string creerLogin(int statut, std::string nom); //ici statut en int, a mettre chez etu/prof ?
